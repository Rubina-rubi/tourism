<?php
defined('BASEPATH') OR exit('No direct script access allowed');
?><!DOCTYPE html>
<!--[if lt IE 7]>      <html class="no-js lt-ie9 lt-ie8 lt-ie7"> <![endif]-->
<!--[if IE 7]>         <html class="no-js lt-ie9 lt-ie8"> <![endif]-->
<!--[if IE 8]>         <html class="no-js lt-ie9"> <![endif]-->
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>Sylhet Tourism</title>
        <meta name="description" content="">


        <!-- Place favicon.ico and apple-touch-icon.png in the root directory -->
        <link href="http://netdna.bootstrapcdn.com/font-awesome/4.1.0/css/font-awesome.min.css" rel="stylesheet">
      <link rel="stylesheet" type="text/css" href="//fonts.googleapis.com/css?family=Lobster+Two" />
            <link href='https://fonts.googleapis.com/css?family=Poppins' rel='stylesheet'>
            <link href='https://fonts.googleapis.com/css?family=Poppins' rel='stylesheet'>
    <link rel="stylesheet" type="text/css" href="<?=base_url()?>asset/css/custom.css" />
    <link rel="stylesheet" href="<?=base_url()?>asset/ukit/css/uikit.min.css" />
    <link rel="stylesheet" href="<?=base_url()?>asset/ukit/css/components/slider.css" />
    <link href="<?=base_url()?>asset/ukit/css/uikit.docs.min.css" rel="stylesheet">
    
    <link rel="stylesheet" href="<?=base_url()?>asset/css/bootstrap.css">
    <link href="<?=base_url()?>asset/css/bootstrap.min.css" rel="stylesheet">
        <link rel="stylesheet" href="<?=base_url()?>asset/css/slicknav.css">
        <link rel="stylesheet" href="<?=base_url()?>asset/css/font-awesome.min.css">
        <link rel="stylesheet" href="<?=base_url()?>asset/css/font-awesome.css">
        <link rel="stylesheet" href="<?=base_url()?>asset/css/normalize.css">
    
        <script src="<?=base_url()?>asset/js/vendor/modernizr-2.6.2.min.js"></script>
    <script type="text/javascript" charset="utf8" src="<?=base_url()?>asset/js/ajaxload.js"></script>
  
    <link rel="stylesheet" href="<?=base_url()?>asset/style.css">
    <link rel="stylesheet" href="<?=base_url()?>asset/responsive.css">

</head>