<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Mod_user_access extends CI_model
	{
		...
		function get_login_info(){			
	        ...
	        $user_details = $this->mod_user_access->system_user_details(false, $su_email);
	        if($user_details){
	            $su_password = $user_details['su_password'];

	            if(crypt($password, $su_password)==$su_password){

					$sess_array = array(
							'is_logged_in' => true,
							'su_id' => $user_details['su_id'],
							'su_first_name' => $user_details['su_first_name'],
							'sut_type_key' => $user_details['sut_type_key']
						);
